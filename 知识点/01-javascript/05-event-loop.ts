/**
 * ============================================================
 * 📚 事件循环（Event Loop）
 * ============================================================
 *
 * 面试考察重点：
 * 1. 事件循环的机制
 * 2. 宏任务和微任务的区别
 * 3. 常见异步任务的分类
 * 4. 经典输出顺序题
 */

// ============================================================
// 1. 为什么需要事件循环？
// ============================================================

/**
 * 📖 JavaScript 是单线程的
 *
 * 原因：JavaScript 最初是为浏览器设计的，用于操作 DOM。
 * 如果多线程同时操作 DOM，会导致冲突。
 *
 * 单线程的问题：如果执行耗时操作，会阻塞后续代码。
 * 解决方案：事件循环（Event Loop）实现异步非阻塞。
 *
 * 📊 单线程 + 异步 = 事件循环
 */

// ============================================================
// 2. 事件循环机制
// ============================================================

/**
 * 📊 浏览器事件循环模型
 *
 * ┌───────────────────────────────────────────────────────────────┐
 * │                         调用栈 (Call Stack)                   │
 * │  ┌─────────────────────────────────────────────────────────┐ │
 * │  │  执行同步代码                                            │ │
 * │  └─────────────────────────────────────────────────────────┘ │
 * │                              │                               │
 * │                              ▼                               │
 * │  ┌─────────────────────────────────────────────────────────┐ │
 * │  │  微任务队列 (Microtask Queue)                           │ │
 * │  │  Promise.then, MutationObserver, queueMicrotask         │ │
 * │  │  【同步代码执行完后，清空所有微任务】                      │ │
 * │  └─────────────────────────────────────────────────────────┘ │
 * │                              │                               │
 * │                              ▼                               │
 * │  ┌─────────────────────────────────────────────────────────┐ │
 * │  │  渲染更新 (可能执行)                                     │ │
 * │  │  requestAnimationFrame → 样式计算 → 布局 → 绘制          │ │
 * │  └─────────────────────────────────────────────────────────┘ │
 * │                              │                               │
 * │                              ▼                               │
 * │  ┌─────────────────────────────────────────────────────────┐ │
 * │  │  宏任务队列 (Macrotask Queue)                           │ │
 * │  │  setTimeout, setInterval, I/O, UI 渲染, requestIdleCallback│
 * │  │  【从队列中取一个宏任务执行】                             │ │
 * │  └─────────────────────────────────────────────────────────┘ │
 * │                              │                               │
 * │                              └──────────► 循环               │
 * └───────────────────────────────────────────────────────────────┘
 */

// ============================================================
// 3. 宏任务与微任务
// ============================================================

/**
 * 📊 宏任务（Macrotask）
 *
 * - script（整体代码）
 * - setTimeout / setInterval
 * - setImmediate（Node.js）
 * - I/O
 * - UI 渲染
 * - requestAnimationFrame
 * - MessageChannel
 *
 * 📊 微任务（Microtask）
 *
 * - Promise.then / catch / finally
 * - async/await（await 后的代码）
 * - MutationObserver
 * - queueMicrotask
 * - process.nextTick（Node.js，优先级最高）
 *
 * 💡 关键区别：
 * - 微任务在当前宏任务执行完后立即执行
 * - 微任务队列会被完全清空后才执行下一个宏任务
 * - 微任务优先级高于宏任务
 */

// ============================================================
// 4. 执行顺序示例
// ============================================================

// 示例 1：基础
console.log('1'); // 同步

setTimeout(() => {
  console.log('2'); // 宏任务
}, 0);

Promise.resolve().then(() => {
  console.log('3'); // 微任务
});

console.log('4'); // 同步

// 输出：1, 4, 3, 2

// 示例 2：微任务中产生微任务
console.log('start');

setTimeout(() => {
  console.log('timeout');
}, 0);

Promise.resolve()
  .then(() => {
    console.log('promise1');
    Promise.resolve().then(() => {
      console.log('promise2');
    });
  })
  .then(() => {
    console.log('promise3');
  });

console.log('end');

// 输出：start, end, promise1, promise2, promise3, timeout
// 解析：微任务队列会被完全清空后才执行宏任务

// ============================================================
// 5. async/await 与事件循环
// ============================================================

/**
 * 📖 async/await 的本质
 *
 * async 函数返回 Promise
 * await 相当于 Promise.then，会让出执行权
 *
 * await expression 等价于：
 * Promise.resolve(expression).then(result => {
 *   // await 后面的代码
 * })
 */

async function async1() {
  console.log('async1 start');
  await async2(); // 相当于 Promise.resolve(async2()).then(...)
  console.log('async1 end'); // 进入微任务队列
}

async function async2() {
  console.log('async2');
}

console.log('script start');

setTimeout(() => {
  console.log('setTimeout');
}, 0);

async1();

new Promise((resolve) => {
  console.log('promise1');
  resolve(undefined);
}).then(() => {
  console.log('promise2');
});

console.log('script end');

/**
 * 输出顺序：
 * 1. script start（同步）
 * 2. async1 start（同步，进入 async1）
 * 3. async2（同步，执行 async2）
 * 4. promise1（同步，Promise 构造函数）
 * 5. script end（同步）
 * 6. async1 end（微任务）
 * 7. promise2（微任务）
 * 8. setTimeout（宏任务）
 */

// ============================================================
// 6. requestAnimationFrame
// ============================================================

/**
 * 📖 requestAnimationFrame (rAF)
 *
 * - 在浏览器下次重绘之前调用
 * - 通常每秒 60 次（16.67ms 一次）
 * - 比 setTimeout 更适合做动画
 *
 * 执行时机：
 * - 在微任务之后，宏任务之前
 * - 在渲染之前执行
 *
 * 注意：rAF 的回调是在所有微任务执行完之后、渲染之前执行
 */

// 动画示例
function animate(element: HTMLElement) {
  let start: number;

  function step(timestamp: number) {
    if (!start) start = timestamp;
    const progress = timestamp - start;

    element.style.transform = `translateX(${Math.min(progress / 10, 200)}px)`;

    if (progress < 2000) {
      requestAnimationFrame(step);
    }
  }

  requestAnimationFrame(step);
}

// ============================================================
// 7. requestIdleCallback
// ============================================================

/**
 * 📖 requestIdleCallback (rIC)
 *
 * - 在浏览器空闲时执行
 * - 适合执行不紧急的任务
 * - React Fiber 的调度借鉴了这个思想
 *
 * 执行时机：
 * - 在一帧的渲染完成后，如果还有剩余时间
 * - 或者设置了 timeout 后强制执行
 */

// 示例
if (typeof requestIdleCallback !== 'undefined') {
  requestIdleCallback(
    (deadline) => {
      // deadline.timeRemaining() 返回当前帧剩余时间
      while (deadline.timeRemaining() > 0) {
        // 执行低优先级任务
        console.log('Idle work');
      }
    },
    { timeout: 1000 } // 最多等 1 秒
  );
}

// ============================================================
// 8. Node.js 事件循环
// ============================================================

/**
 * 📊 Node.js 事件循环（不同于浏览器）
 *
 *    ┌───────────────────────────┐
 * ┌─>│           timers          │  setTimeout, setInterval
 * │  └─────────────┬─────────────┘
 * │  ┌─────────────┴─────────────┐
 * │  │     pending callbacks     │  I/O 回调
 * │  └─────────────┬─────────────┘
 * │  ┌─────────────┴─────────────┐
 * │  │       idle, prepare       │  内部使用
 * │  └─────────────┬─────────────┘
 * │  ┌─────────────┴─────────────┐
 * │  │           poll            │  获取新的 I/O 事件
 * │  └─────────────┬─────────────┘
 * │  ┌─────────────┴─────────────┐
 * │  │           check           │  setImmediate
 * │  └─────────────┬─────────────┘
 * │  ┌─────────────┴─────────────┐
 * └──┤      close callbacks      │  socket.on('close')
 *    └───────────────────────────┘
 *
 * 每个阶段之间都会执行微任务队列
 * process.nextTick 优先级高于其他微任务
 */

// Node.js 示例
// setTimeout(() => {
//   console.log('timeout');
// }, 0);

// setImmediate(() => {
//   console.log('immediate');
// });

// 输出顺序不确定（取决于事件循环进入时机）

// 但在 I/O 回调中，setImmediate 总是先执行
// const fs = require('fs');
// fs.readFile(__filename, () => {
//   setTimeout(() => console.log('timeout'), 0);
//   setImmediate(() => console.log('immediate'));
// });
// 输出：immediate, timeout

// ============================================================
// 9. 高频面试题
// ============================================================

/**
 * 题目 1：经典输出顺序
 */
console.log('1');

setTimeout(function () {
  console.log('2');
  new Promise(function (resolve) {
    console.log('3');
    resolve(undefined);
  }).then(function () {
    console.log('4');
  });
}, 0);

new Promise(function (resolve) {
  console.log('5');
  resolve(undefined);
}).then(function () {
  console.log('6');
});

setTimeout(function () {
  console.log('7');
  new Promise(function (resolve) {
    console.log('8');
    resolve(undefined);
  }).then(function () {
    console.log('9');
  });
}, 0);

console.log('10');

/**
 * 输出：1, 5, 10, 6, 2, 3, 4, 7, 8, 9
 *
 * 解析：
 * 1. 同步代码：1, 5, 10
 * 2. 微任务：6
 * 3. 第一个宏任务：2, 3
 * 4. 微任务：4
 * 5. 第二个宏任务：7, 8
 * 6. 微任务：9
 */

/**
 * 题目 2：async/await 输出顺序
 */
async function async3() {
  console.log('async3 start');
  await async4();
  console.log('async3 end');
  await async5();
  console.log('async3 final');
}

async function async4() {
  console.log('async4');
}

async function async5() {
  console.log('async5');
}

console.log('script start');
async3();
console.log('script end');

/**
 * 输出：
 * script start
 * async3 start
 * async4
 * script end
 * async3 end
 * async5
 * async3 final
 */

/**
 * 题目 3：Promise 链式调用
 */
Promise.resolve()
  .then(() => {
    console.log(0);
    return Promise.resolve(4);
  })
  .then((res) => {
    console.log(res);
  });

Promise.resolve()
  .then(() => {
    console.log(1);
  })
  .then(() => {
    console.log(2);
  })
  .then(() => {
    console.log(3);
  })
  .then(() => {
    console.log(5);
  })
  .then(() => {
    console.log(6);
  });

/**
 * 输出：0, 1, 2, 3, 4, 5, 6
 *
 * 解析：
 * - return Promise.resolve(4) 会产生额外的微任务
 * - V8 引擎对 Promise 返回 Promise 有特殊处理
 * - 相当于额外添加了两个微任务
 */

// ============================================================
// 10. 实战应用
// ============================================================

// 10.1 实现微任务
function nextTick(callback: () => void) {
  if (typeof queueMicrotask === 'function') {
    queueMicrotask(callback);
  } else if (typeof Promise !== 'undefined') {
    Promise.resolve().then(callback);
  } else if (typeof MutationObserver !== 'undefined') {
    // MutationObserver 实现（兼容旧浏览器）
    const observer = new MutationObserver(callback);
    const textNode = document.createTextNode('');
    observer.observe(textNode, { characterData: true });
    textNode.data = '1';
  } else {
    // 降级到宏任务
    setTimeout(callback, 0);
  }
}

// 10.2 实现时间切片（避免长任务阻塞）
function timeSlice(tasks: (() => void)[], yieldTime = 5) {
  const queue = [...tasks];

  function runTask() {
    const start = performance.now();

    // 在一个时间片内尽可能执行多个任务
    while (queue.length > 0 && performance.now() - start < yieldTime) {
      const task = queue.shift();
      task?.();
    }

    // 还有任务，让出主线程
    if (queue.length > 0) {
      // 使用 MessageChannel 创建宏任务
      const channel = new MessageChannel();
      channel.port1.onmessage = runTask;
      channel.port2.postMessage(null);
    }
  }

  runTask();
}

// 10.3 判断是否在主线程
function isMainThread(): boolean {
  return typeof window !== 'undefined' && typeof window.document !== 'undefined';
}

// ============================================================
// 🔥 高频面试追问（增强版）
// ============================================================

/**
 * 追问 1：setTimeout(fn, 0) 真的是 0 毫秒吗？
 *
 * 不是！
 * - HTML5 规范：嵌套超过 5 层，最小延迟 4ms
 * - 浏览器实现：通常最小 1ms-4ms
 * - 非活动标签页：可能被节流到 1000ms+
 *
 * 💡 如果需要更快的异步，用 queueMicrotask 或 MessageChannel
 */

/**
 * 追问 2：Promise.then 一定是异步吗？
 *
 * 是的！即使 Promise 已经 resolved
 * then 回调也会被放入微任务队列
 *
 * Promise.resolve().then(() => console.log(1));
 * console.log(2);
 * // 输出：2, 1
 */

/**
 * 追问 3：async/await 如何影响事件循环？
 *
 * async 函数返回 Promise
 * await 后面的代码相当于 then 回调
 *
 * async function foo() {
 *   console.log(1);        // 同步
 *   await Promise.resolve();
 *   console.log(2);        // 微任务
 * }
 * foo();
 * console.log(3);
 * // 输出：1, 3, 2
 */

/**
 * 追问 4：requestAnimationFrame 和 setTimeout 的区别？
 *
 * setTimeout：宏任务，时间不精确
 * requestAnimationFrame：
 * - 渲染前执行
 * - 与屏幕刷新率同步（60fps = 16.6ms）
 * - 页面不可见时暂停
 * - 适合做动画
 */

/**
 * 追问 5：Node.js 事件循环和浏览器有什么区别？
 *
 * Node.js 有 6 个阶段：
 * timers → I/O callbacks → idle/prepare → poll → check → close
 *
 * 特殊 API：
 * - process.nextTick：比微任务更优先
 * - setImmediate：check 阶段执行
 *
 * ⚠️ Node.js 11+ 行为已与浏览器基本一致
 */

/**
 * 追问 6：如何让浏览器在两个宏任务之间渲染？
 *
 * 方案 1：setTimeout
 * setTimeout(() => { /* 第一个任务 */ }, 0);
 * setTimeout(() => { /* 第二个任务 */ }, 0);
 *
 * 方案 2：MessageChannel（更快）
 * const channel = new MessageChannel();
 * channel.port1.onmessage = () => { /* 任务 */ };
 * channel.port2.postMessage(null);
 *
 * ⚠️ 微任务不会让出主线程给渲染
 */

export { nextTick, timeSlice, isMainThread };

