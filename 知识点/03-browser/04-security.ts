/**
 * ============================================================
 * 📚 浏览器安全
 * ============================================================
 *
 * 面试考察重点：
 * 1. 同源策略
 * 2. XSS 攻击与防御
 * 3. CSRF 攻击与防御
 * 4. 其他安全问题
 */

// ============================================================
// 1. 同源策略（Same-Origin Policy）
// ============================================================

/**
 * 📖 什么是同源策略？
 *
 * 同源策略是浏览器的安全基石，限制不同源的文档/脚本之间的交互。
 *
 * 同源：协议 + 域名 + 端口 完全相同
 *
 * https://www.example.com:443/path
 *   │          │           │
 * 协议       域名        端口
 *
 * 📊 同源判断示例（相对于 https://www.example.com/page）
 *
 * ┌─────────────────────────────────────┬──────────┬──────────┐
 * │ URL                                 │ 是否同源  │ 原因     │
 * ├─────────────────────────────────────┼──────────┼──────────┤
 * │ https://www.example.com/other       │ ✅ 同源  │          │
 * │ http://www.example.com/page         │ ❌ 不同源 │ 协议不同 │
 * │ https://api.example.com/page        │ ❌ 不同源 │ 域名不同 │
 * │ https://www.example.com:8080/page   │ ❌ 不同源 │ 端口不同 │
 * └─────────────────────────────────────┴──────────┴──────────┘
 */

/**
 * 📊 同源策略的限制
 *
 * 1. DOM 访问限制：
 *    - 不能读取跨域 iframe 的 DOM
 *
 * 2. Cookie/Storage 限制：
 *    - 不能读取跨域的 Cookie、LocalStorage
 *
 * 3. AJAX 限制：
 *    - 不能向跨域地址发送 AJAX 请求（或请求被阻止响应）
 *
 * 4. 例外（可跨域）：
 *    - <script src>
 *    - <link href>
 *    - <img src>
 *    - <video> / <audio>
 *    - <iframe>（可加载，但不能操作 DOM）
 *    - @font-face
 */

/**
 * 📊 跨域解决方案
 *
 * 1. CORS（跨域资源共享）- 推荐
 * 2. JSONP
 * 3. 代理服务器
 * 4. postMessage
 * 5. WebSocket
 * 6. document.domain（已废弃）
 */

// ============================================================
// 2. CORS（跨域资源共享）
// ============================================================

/**
 * 📖 CORS 工作原理
 *
 * CORS 通过 HTTP 响应头告诉浏览器是否允许跨域请求。
 *
 * 📊 简单请求 vs 预检请求
 *
 * 简单请求条件（同时满足）：
 * - 方法：GET、POST、HEAD
 * - 头部：Accept、Accept-Language、Content-Language、Content-Type
 * - Content-Type：text/plain、multipart/form-data、application/x-www-form-urlencoded
 *
 * 不满足条件的是预检请求（Preflight）
 */

/**
 * 📊 简单请求流程
 *
 * 浏览器                              服务器
 *   │                                   │
 *   │  GET /api/data                    │
 *   │  Origin: https://example.com      │
 *   │ ─────────────────────────────────►│
 *   │                                   │
 *   │  200 OK                           │
 *   │  Access-Control-Allow-Origin: *   │
 *   │◄───────────────────────────────── │
 *   │                                   │
 */

/**
 * 📊 预检请求流程
 *
 * 浏览器                              服务器
 *   │                                   │
 *   │  OPTIONS /api/data                │ ← 预检请求
 *   │  Origin: https://example.com      │
 *   │  Access-Control-Request-Method: PUT│
 *   │  Access-Control-Request-Headers: X-Custom│
 *   │ ─────────────────────────────────►│
 *   │                                   │
 *   │  200 OK                           │ ← 预检响应
 *   │  Access-Control-Allow-Origin: https://example.com
 *   │  Access-Control-Allow-Methods: PUT, POST, GET
 *   │  Access-Control-Allow-Headers: X-Custom
 *   │  Access-Control-Max-Age: 86400    │ ← 预检结果缓存
 *   │◄───────────────────────────────── │
 *   │                                   │
 *   │  PUT /api/data                    │ ← 实际请求
 *   │  Origin: https://example.com      │
 *   │  X-Custom: value                  │
 *   │ ─────────────────────────────────►│
 *   │                                   │
 *   │  200 OK                           │
 *   │  Access-Control-Allow-Origin: https://example.com
 *   │◄───────────────────────────────── │
 */

/**
 * 📊 CORS 响应头
 *
 * - Access-Control-Allow-Origin: * | <origin>
 * - Access-Control-Allow-Methods: GET, POST, PUT, DELETE
 * - Access-Control-Allow-Headers: X-Custom, Content-Type
 * - Access-Control-Allow-Credentials: true（允许携带 Cookie）
 * - Access-Control-Expose-Headers: X-Custom（暴露自定义响应头）
 * - Access-Control-Max-Age: 86400（预检结果缓存时间）
 *
 * 注意：
 * - Allow-Origin 为 * 时，不能携带 Cookie
 * - 需要携带 Cookie 时，必须指定具体的 Origin
 */

// ============================================================
// 3. XSS（跨站脚本攻击）
// ============================================================

/**
 * 📖 什么是 XSS？
 *
 * XSS（Cross-Site Scripting）：攻击者在网页中注入恶意脚本，
 * 当用户访问时执行，窃取用户信息或执行恶意操作。
 *
 * 📊 XSS 类型
 *
 * 1. 存储型 XSS（最危险）
 *    - 恶意脚本存储在服务器（数据库）
 *    - 用户访问页面时从服务器获取并执行
 *    - 场景：评论、文章、用户资料
 *
 * 2. 反射型 XSS
 *    - 恶意脚本在 URL 参数中
 *    - 服务器将参数直接返回页面
 *    - 场景：搜索结果、错误消息
 *
 * 3. DOM 型 XSS
 *    - 前端 JS 直接将不可信数据插入 DOM
 *    - 不经过服务器
 *    - 场景：innerHTML、document.write、eval
 */

/**
 * 📊 XSS 攻击示例
 */

// 存储型 XSS
// 攻击者提交评论：<script>steal(document.cookie)</script>
// 其他用户查看评论时，脚本执行

// 反射型 XSS
// 攻击者构造链接：https://example.com/search?q=<script>alert(1)</script>
// 服务器返回：搜索结果：<script>alert(1)</script>

// DOM 型 XSS
// const query = new URLSearchParams(location.search).get('q');
// document.getElementById('result').innerHTML = query;
// 攻击者构造：?q=<img src=x onerror=alert(1)>

/**
 * 📊 XSS 防御方案
 */

// 1. 输入验证和过滤
function sanitizeInput(input: string): string {
  // 白名单验证
  return input.replace(/[^a-zA-Z0-9\s]/g, '');
}

// 2. 输出编码（HTML 实体编码）
function escapeHtml(str: string): string {
  const escapeMap: Record<string, string> = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
    '"': '&quot;',
    "'": '&#x27;',
    '/': '&#x2F;',
  };
  return str.replace(/[&<>"'/]/g, (char) => escapeMap[char]);
}

// 3. 使用安全的 API
// ❌ 危险
// element.innerHTML = userInput;
// document.write(userInput);
// eval(userInput);

// ✅ 安全
// element.textContent = userInput;
// element.setAttribute('value', userInput);

// 4. CSP（内容安全策略）
const cspExample = `
  <!-- HTTP 响应头 -->
  Content-Security-Policy:
    default-src 'self';                    // 只允许同源资源
    script-src 'self' 'nonce-abc123';      // 脚本需要 nonce
    style-src 'self' 'unsafe-inline';       // 允许内联样式
    img-src *;                              // 图片可从任意来源
    connect-src 'self' https://api.example.com;  // AJAX 限制

  <!-- HTML 标签 -->
  <meta http-equiv="Content-Security-Policy" content="default-src 'self'">

  <!-- 带 nonce 的脚本 -->
  <script nonce="abc123">
    // 这个脚本可以执行
  </script>
`;

// 5. HttpOnly Cookie
// 设置 HttpOnly 的 Cookie 不能被 JavaScript 访问
// Set-Cookie: session=xxx; HttpOnly; Secure; SameSite=Strict

// ============================================================
// 4. CSRF（跨站请求伪造）
// ============================================================

/**
 * 📖 什么是 CSRF？
 *
 * CSRF（Cross-Site Request Forgery）：攻击者诱导用户在已登录的网站上
 * 执行非预期的操作。
 *
 * 📊 CSRF 攻击流程
 *
 * 1. 用户登录了 bank.com，浏览器保存了 Cookie
 * 2. 用户访问恶意网站 evil.com
 * 3. evil.com 向 bank.com/transfer 发起请求
 * 4. 浏览器自动携带 bank.com 的 Cookie
 * 5. bank.com 认为是用户操作，执行转账
 *
 *    用户浏览器
 *        │
 *   ┌────┴────┐
 *   ▼         ▼
 * bank.com  evil.com
 *   │         │
 *   │         │ <img src="bank.com/transfer?to=hacker&amount=1000">
 *   │         │
 *   │◄────────┘ 自动发起请求，携带 Cookie
 *   │
 *   ▼
 * 执行转账 😱
 */

/**
 * 📊 CSRF 防御方案
 */

// 1. SameSite Cookie
const sameSiteCookie = `
  Set-Cookie: session=xxx; SameSite=Strict
  // Strict: 完全禁止第三方网站发送
  // Lax: 允许部分安全请求（GET 链接）
  // None: 不限制（需要 Secure）
`;

// 2. CSRF Token
const csrfToken = `
  <!-- 服务器生成 token，嵌入表单 -->
  <form action="/transfer" method="POST">
    <input type="hidden" name="csrf_token" value="random-token-123">
    <input type="text" name="amount">
    <button type="submit">转账</button>
  </form>

  // 服务器验证 token 是否有效
  // 攻击者无法获取 token，请求会失败
`;

// 3. 验证 Referer/Origin
const refererCheck = `
  // 服务器检查请求来源
  const referer = req.headers.referer;
  const origin = req.headers.origin;

  if (!referer?.startsWith('https://bank.com') &&
      !origin?.startsWith('https://bank.com')) {
    return res.status(403).send('CSRF detected');
  }
`;

// 4. 双重 Cookie 验证
const doubleCookie = `
  // 服务器设置 Cookie
  Set-Cookie: csrf=random-value

  // 前端请求时，将 Cookie 值放入请求头
  fetch('/api', {
    headers: {
      'X-CSRF-Token': getCookie('csrf')
    }
  })

  // 服务器验证 Cookie 和 Header 是否匹配
  // 攻击者无法读取 Cookie，无法设置正确的 Header
`;

// ============================================================
// 5. 其他安全问题
// ============================================================

/**
 * 📊 点击劫持（Clickjacking）
 *
 * 攻击方式：
 * - 将目标网站嵌入透明 iframe
 * - 诱导用户点击，实际点击的是 iframe 中的按钮
 *
 * 防御：
 * - X-Frame-Options: DENY / SAMEORIGIN
 * - CSP: frame-ancestors 'none'
 */

/**
 * 📊 中间人攻击（MITM）
 *
 * 攻击方式：
 * - 攻击者在用户和服务器之间拦截/篡改数据
 *
 * 防御：
 * - 使用 HTTPS
 * - HSTS（强制 HTTPS）
 * - 证书锁定
 */

/**
 * 📊 安全响应头
 */
const securityHeaders = `
  // 防止点击劫持
  X-Frame-Options: DENY

  // 禁止 MIME 类型嗅探
  X-Content-Type-Options: nosniff

  // 启用 XSS 过滤器（已废弃，用 CSP 替代）
  X-XSS-Protection: 1; mode=block

  // 强制 HTTPS
  Strict-Transport-Security: max-age=31536000; includeSubDomains

  // 控制 Referer
  Referrer-Policy: strict-origin-when-cross-origin

  // 权限策略
  Permissions-Policy: geolocation=(), microphone=()
`;

// ============================================================
// 6. 高频面试题
// ============================================================

/**
 * 题目 1：什么是同源策略？如何跨域？
 *
 * 同源策略：限制不同源的文档/脚本交互。
 * 同源 = 协议 + 域名 + 端口 相同
 *
 * 跨域方案：
 * 1. CORS：服务器设置 Access-Control-Allow-Origin
 * 2. JSONP：利用 script 标签（只支持 GET）
 * 3. 代理服务器：同源服务器转发请求
 * 4. postMessage：窗口间通信
 * 5. WebSocket：没有同源限制
 */

/**
 * 题目 2：XSS 和 CSRF 的区别？
 *
 * XSS（跨站脚本）：
 * - 注入恶意脚本到页面
 * - 脚本在用户浏览器执行
 * - 窃取信息、劫持会话
 * - 防御：输入验证、输出编码、CSP
 *
 * CSRF（跨站请求伪造）：
 * - 诱导用户发起非预期请求
 * - 利用用户已登录的身份
 * - 执行非预期操作
 * - 防御：CSRF Token、SameSite Cookie、验证来源
 */

/**
 * 题目 3：如何防范 XSS 攻击？
 *
 * 1. 输入验证：白名单过滤
 * 2. 输出编码：HTML 实体编码
 * 3. 安全 API：textContent 代替 innerHTML
 * 4. CSP：限制脚本来源
 * 5. HttpOnly Cookie：防止 JS 访问
 * 6. 框架自动转义：React、Vue 等
 */

/**
 * 题目 4：CORS 简单请求和预检请求的区别？
 *
 * 简单请求：
 * - GET/POST/HEAD
 * - 只有简单头部
 * - 直接发送请求
 *
 * 预检请求：
 * - 其他方法（PUT、DELETE）
 * - 自定义头部
 * - 先发 OPTIONS 请求
 * - 服务器允许后再发实际请求
 */

export {
  escapeHtml,
  sanitizeInput,
  cspExample,
  sameSiteCookie,
  securityHeaders,
};

