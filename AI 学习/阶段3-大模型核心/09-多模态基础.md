...
> 了解 Diffusion 模型、CLIP、多模态 LLM 的基本原理

---

## 目录

1. [多模态 AI 概览](#1-多模态-ai-概览)
2. [CLIP 图文对齐](#2-clip-图文对齐)
3. [Diffusion 扩散模型](#3-diffusion-扩散模型)
4. [多模态 LLM](#4-多模态-llm)
5. [练习题](#5-练习题)
...
多模态：处理多种数据类型
  - 图文：CLIP、DALL-E、Stable Diffusion
  - 文本+图像理解：GPT-4V、LLaVA、Qwen-VL
  - 文本+音频：GPT-4o
  - 全模态：Gemini
```
...
```
1. 图文匹配：给定图像和文本，判断是否匹配
2. 图像生成：根据文本生成图像（Text-to-Image）
3. 图像理解：描述图像内容（Image Captioning）
4. 视觉问答：根据图像回答问题（VQA）
5. 图像编辑：根据指令编辑图像
```
...
# 多轮对话
response, history = model.chat(tokenizer, query='图片中有什么动物？', history=history)
print(response)
```
...
"""
print("多模态 LLM 需要较大的显存，建议在 GPU 上运行")
```
...
学完本节后，继续学习 [10-项目-BERT文本分类.md](./10-项目-BERT文本分类.md)
