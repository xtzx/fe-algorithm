...
class SemanticSearchEngine:
    ...
    def __init__(self, model_name: str = 'all-MiniLM-L6-v2'):
        ...
        # 初始化 Faiss 索引（使用内积，配合归一化向量等价于余弦相似度）
    ...
    def add_documents(self, texts: List[str], metadatas: Optional[List[Dict]] = None) -> List[int]:
        ...
        print(f"添加了 {len(texts)} 个文档，总计 {len(self.documents)} 个")
        return doc_ids
    ...
    def load(self, path: str):
        ...
        print(f"从 {path} 加载了 {len(self.documents)} 个文档")
...
@app.get("/")
def root():
    return {"message": "语义搜索引擎 API", "stats": engine.get_stats()}
...
def delete_document(doc_id: int):
    ...
    return {"message": f"成功删除文档 {doc_id}"}
...
def load_index(path: str = "./search_index"):
    ...
    try:
        engine.load(path)
        return {"message": f"索引已从 {path} 加载", "stats": engine.get_stats()}
...
response = requests.post(f"{BASE_URL}/documents", json={
    "documents": [
        ...
        {"text": "机器学习是人工智能的子领域", "metadata": {"lang": "zh"}}
    ]
})
print(response.json())
...
class HybridSearchEngine(SemanticSearchEngine):
    ...
    def search(self, query, top_k=5, threshold=0.0):
        ...
        semantic_scores = (semantic_scores[0] + 1) / 2  # 归一化到 [0, 1]
...
继续 [12-项目-手写极简GPT.md](./12-项目-手写极简GPT.md)
