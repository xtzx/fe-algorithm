...
# R²
axes[2].barh(results_df['Model'], results_df['Test R²'], color='orange')
...
# 对 Gradient Boosting 进行调优
print("对 Gradient Boosting 进行网格搜索调优...")
...
print(f"最佳 CV RMSE: {-grid_search.best_score_:.4f}")
...
print("""
项目总结：
1. 数据包含 8 个特征，20640 个样本
2. 目标变量（房价中位数）呈右偏分布，最高值被截断在 5.0
3. 最重要的特征是 MedInc（收入中位数）和地理位置
4. Gradient Boosting 和 Random Forest 表现最好
5. 通过超参数调优可以进一步提升性能
...
- [ ] 使用 XGBoost 和 LightGBM，对比性能
- [ ] 添加特征交互（如 `Rooms * Bedrooms`）
- [ ] 对目标变量做对数变换，观察效果
- [ ] 使用 Optuna 进行更智能的超参数调优
- [ ] 分析预测错误较大的样本，找出原因
...
完成本项目后，继续挑战 [12-项目-泰坦尼克.md](./12-项目-泰坦尼克.md)
