...
print(f"系数 (w1): {model.coef_[0]:.4f}")
print(f"学到的方程: y = {model.intercept_:.2f} + {model.coef_[0]:.2f} * x")
...
print(f"手动实现 - 截距: {my_model.intercept_:.4f}, 系数: {my_model.coef_[0]:.4f}")
print(f"手动实现 - R²: {my_model.score(X_test, y_test):.4f}")

# sklearn
sk_model = LinearRegression()
sk_model.fit(X_train, y_train)
print(f"sklearn   - 截距: {sk_model.intercept_:.4f}, 系数: {sk_model.coef_[0]:.4f}")
...
多项式回归用于拟合非线性数据，通过构造多项式特征来扩展线性模型
...
其中 σ 是 Sigmoid 函数，将任意实数映射到 (0, 1)
...
for i in sample_indices:
    ...
    true = y_test[i]
    print(f"样本 {i}: P(0)={prob[0]:.3f}, P(1)={prob[1]:.3f} → 预测={pred}, 真实={true}")
...
1. 用线性回归拟合一组数据，画出回归线
2. 用逻辑回归做二分类，画出决策边界
3. 比较 Ridge 和 Lasso 的系数差异

### 进阶练习

4. 用多项式回归拟合非线性数据，找到最佳的 degree
5. 用逻辑回归做鸢尾花三分类，画出混淆矩阵
...
print(f"最佳 degree: {best_degree}")
print(f"最佳 CV MSE: {min(cv_scores):.4f}")
...
学完本节后，继续学习 [05-树模型与集成学习.md](./05-树模型与集成学习.md)
