...
**学习曲线解读**：
- **欠拟合**：训练误差和验证误差都很高，且差距小
- **过拟合**：训练误差低，验证误差高，差距大
- **良好拟合**：两条曲线最终收敛到一个较低的值
...
# 不同复杂度的模型
max_depths = [2, 5, 10, 20, None]  # None 表示不限制深度
...
for alpha in alphas:
    ...
    print(f"Alpha = {alpha}: {n_nonzero}/{len(model.coef_)} 个非零系数")
```
...
model = ElasticNet(
    alpha=1.0,       # 总正则化强度
    l1_ratio=0.5,    # L1 比例 (0=Ridge, 1=Lasso)
    max_iter=10000
)
model.fit(X_train, y_train)

print(f"非零系数: {np.sum(model.coef_ != 0)}/{len(model.coef_)}")
...
for ratio in l1_ratios:
    ...
    print(f"l1_ratio={ratio}: 非零系数={n_nonzero}, Test MSE={test_mse:.4f}")
```
...
print(f"最佳 CV MSE: {cv_scores[best_idx]:.4f}")
```
...
print(f"原始特征 ({X.shape[1]}): {scores_original.mean():.4f}")
...
print(f"选择后特征 ({X_selected.shape[1]}): {scores_selected.mean():.4f}")
```
...
model_log2 = RandomForestClassifier(
    n_estimators=100,
    max_features='log2',  # 使用 log2(n_features) 个特征
    random_state=42
)
...
for name, model in [('全部特征', model_full), ('sqrt特征', model_sqrt), ('log2特征', model_log2)]:
...
1. 绘制学习曲线，判断模型是过拟合还是欠拟合
2. 比较 Ridge 和 Lasso 在同一数据集上的表现
3. 使用 5-Fold 交叉验证评估模型

### 进阶练习

4. 使用交叉验证选择 Ridge 的最佳 alpha
5. 对一个高维数据集，使用 Lasso 进行特征选择
...
学完本节后，继续学习 [04-线性模型.md](./04-线性模型.md)
