...
1. [无监督学习概述](#1-无监督学习概述)
2. [K-Means 聚类](#2-k-means-聚类)
3. [层次聚类](#3-层次聚类)
4. [DBSCAN 聚类](#4-dbscan-聚类)
5. [PCA 降维](#5-pca-降维)
6. [t-SNE 和 UMAP](#6-t-sne-和-umap)
7. [练习题](#7-练习题)
...
for n in n_components_list:
    ...
    print(f"n_components={n}: {scores.mean():.4f} ± {scores.std():.4f}")
...
print(f"Full features ({X.shape[1]}): {scores_full.mean():.4f} ± {scores_full.std():.4f}")
```
...
1. 用 K-Means 对一个数据集进行聚类，使用肘部法则选择最佳 K
2. 用 PCA 将高维数据降到 2D 并可视化
3. 比较 K-Means 和 DBSCAN 在月牙形数据上的效果

### 进阶练习

4. 用 t-SNE 可视化手写数字数据集
5. 结合 PCA 和聚类：先降维再聚类，观察效果
...
# 先 PCA 降维
pca = PCA(n_components=50)  # 保留 50 个主成分
X_pca = pca.fit_transform(X)

print(f"PCA 后累计方差: {pca.explained_variance_ratio_.sum():.2%}")

# 在降维后的数据上聚类
kmeans = KMeans(n_clusters=10, random_state=42, n_init=10)
labels = kmeans.fit_predict(X_pca)

print(f"轮廓系数: {silhouette_score(X_pca, labels):.4f}")
...
学完本节后，继续学习 [08-超参数调优.md](./08-超参数调优.md)
