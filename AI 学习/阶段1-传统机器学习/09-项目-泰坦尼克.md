...
# Cabin：缺失太多，转换为是否有舱位号
data['HasCabin'] = data['Cabin'].notna().astype(int)

print(f"处理后缺失值: {data[['Age', 'Embarked', 'Fare']].isnull().sum().sum()}")
...
print(f"  原始 Title 分布: {data['Title'].value_counts().to_dict()}")
...
print(f"  合并后 Title: {data['Title'].value_counts().to_dict()}")
...
print(f"训练集: {X_train.shape[0]} ({y_train.mean():.2%} 生存)")
print(f"测试集: {X_test.shape[0]} ({y_test.mean():.2%} 生存)")
...
print(f"最佳 CV Accuracy: {grid_search.best_score_:.4f}")
...
1. 数据洞察：
   - 女性生存率(74%)远高于男性(19%)
   - 1等舱生存率(63%)最高，3等舱最低(24%)
   - 儿童生存率较高

2. 重要特征：
   - 性别(Sex)是最重要的特征
   - 舱位(Pclass)、票价(Fare)也很重要
   - 家庭大小有一定影响
...
- [ ] 使用 XGBoost/LightGBM，对比性能
- [ ] 创建更多特征（如姓名长度、舱位号开头字母等）
- [ ] 尝试特征交叉（如 Sex * Pclass）
- [ ] 使用 Stacking 集成多个模型
- [ ] 提交到 Kaggle 竞赛，查看实际排名
