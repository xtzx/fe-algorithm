...
print("标准化后:")
print(f"  均值: {X_scaled.mean(axis=0).round(4)}")
print(f"  标准差: {X_scaled.std(axis=0).round(4)}")
...
# 使用最佳 K 进行聚类
optimal_k = 4  # 可以根据肘部图和业务理解选择
kmeans = KMeans(n_clusters=optimal_k, random_state=42, n_init=10)
df['Cluster'] = kmeans.fit_predict(X_scaled)

print(f"选择 K = {optimal_k}")
...
for cluster in range(optimal_k):
    ...
    print(f"\n【Cluster {cluster}】- {count} 人 ({pct:.1f}%)")
    print("-" * 40)
    print(f"  平均年龄: {profile['Age']:.0f} 岁")
    print(f"  平均年收入: ${profile['Annual_Income']:,.0f}")
    print(f"  消费评分: {profile['Spending_Score']:.0f}")
    print(f"  购买频率: {profile['Purchase_Frequency']:.0f} 次/月")
    print(f"  平均客单价: ${profile['Avg_Transaction_Value']:.0f}")
    print(f"  会员年限: {profile['Membership_Years']:.1f} 年")
...
print(f"DBSCAN: {n_clusters_dbscan} 个聚类, {(df['DBSCAN_Cluster'] == -1).sum()} 个噪声点")
...
完成所有项目后，进入 [14-自测清单.md](./14-自测清单.md) 检验学习成果
