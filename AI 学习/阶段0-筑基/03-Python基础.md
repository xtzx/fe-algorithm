...
## 🔄 JS vs Python 快速对照表（前端开发者必看）

> 如果你有 JavaScript 背景，这个表格帮你快速上手 Python！

### 语法对比

| 特性 | JavaScript | Python |
|------|------------|--------|
| **变量声明** | `const x = 1` / `let y = 2` | `x = 1`（无关键字） |
| **常量** | `const` | 约定用大写 `PI = 3.14` |
...
| `{}` Object | `{}` dict | 键值对 |
| `new Set()` | `set()` | 集合 |
| `new Map()` | `dict` | Python dict 即是 Map |
| `null` | `None` | 空值 |
...
# Python
numbers = [1, 2, 3, 4, 5]
doubled = [x * 2 for x in numbers]  # 列表推导式
...
| 坑 | JS 行为 | Python 行为 |
|----|---------|-------------|
| **缩进** | 仅影响可读性 | **语法要求！错误缩进会报错** |
| **真值判断** | `0`, `""`, `[]` → falsy | `0`, `""`, `[]`, `{}` → falsy |
| **字符串索引** | 负数返回 undefined | 负数从末尾计数 `s[-1]` |
| **类型转换** | 隐式转换（`"1" + 1 = "11"`） | 严格（`"1" + 1` 报错） |
| **this/self** | 动态绑定 | 显式传入 `self` |
| **分号** | 可选但常用 | 不使用 |
...
学完本节后，继续学习 [04-Python进阶.md](./04-Python进阶.md)
