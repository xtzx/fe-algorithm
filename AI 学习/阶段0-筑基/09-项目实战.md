# 🚀 09 - 项目实战

> 动手实践，巩固所学知识

---

## 项目列表

1. [项目 A：CSV 数据分析与可视化](#项目-a-csv-数据分析与可视化)
2. [项目 B：向量相似度搜索](#项目-b-向量相似度搜索)

---

## 项目 A：CSV 数据分析与可视化

### 项目目标

用 Pandas 分析一个数据集，进行数据清洗、统计分析和可视化。

### 完整代码

```python
"""
项目 A：数据分析与可视化
目标：分析销售数据，发现业务洞察
"""

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns

# 设置显示
pd.set_option('display.max_columns', None)
plt.rcParams['figure.figsize'] = (10, 6)
plt.rcParams['font.size'] = 12

print("=" * 60)
print("1. 创建模拟数据")
print("=" * 60)

# 生成模拟销售数据
np.random.seed(42)
n_records = 500

# 生成数据
dates = pd.date_range('2023-01-01', periods=365, freq='D')
data = {
    'date': np.random.choice(dates, n_records),
    'product': np.random.choice(['产品A', '产品B', '产品C', '产品D'], n_records),
    'category': np.random.choice(['电子', '服装', '食品'], n_records),
    'region': np.random.choice(['华东', '华南', '华北', '西部'], n_records),
    'sales': np.random.randint(100, 10000, n_records),
    'quantity': np.random.randint(1, 100, n_records),
    'discount': np.random.uniform(0, 0.3, n_records)
}

df = pd.DataFrame(data)

# 添加一些缺失值
df.loc[np.random.choice(df.index, 20), 'sales'] = np.nan
df.loc[np.random.choice(df.index, 10), 'region'] = np.nan

# 添加计算列
df['revenue'] = df['sales'] * df['quantity'] * (1 - df['discount'])
df['month'] = df['date'].dt.month
df['weekday'] = df['date'].dt.dayofweek

print(f"数据形状: {df.shape}")
print(f"\n前5行数据:")
print(df.head())

print("\n" + "=" * 60)
print("2. 数据探索")
print("=" * 60)

print("\n数据信息:")
print(df.info())

print("\n数值列统计:")
print(df.describe())

print("\n缺失值统计:")
print(df.isnull().sum())

print("\n各列唯一值数量:")
print(df.nunique())

print("\n" + "=" * 60)
print("3. 数据清洗")
print("=" * 60)

# 处理缺失值
df_clean = df.copy()

# 数值列用中位数填充
df_clean['sales'] = df_clean['sales'].fillna(df_clean['sales'].median())

# 类别列用众数填充
df_clean['region'] = df_clean['region'].fillna(df_clean['region'].mode()[0])

# 重新计算 revenue
df_clean['revenue'] = df_clean['sales'] * df_clean['quantity'] * (1 - df_clean['discount'])

print(f"清洗后缺失值: {df_clean.isnull().sum().sum()}")

print("\n" + "=" * 60)
print("4. 数据分析")
print("=" * 60)

# 4.1 基本统计
print("\n=== 基本统计 ===")
print(f"总销售额: ¥{df_clean['revenue'].sum():,.0f}")
print(f"平均订单金额: ¥{df_clean['revenue'].mean():,.0f}")
print(f"订单数量: {len(df_clean)}")

# 4.2 按产品分析
print("\n=== 按产品分析 ===")
product_stats = df_clean.groupby('product').agg({
    'revenue': ['sum', 'mean', 'count'],
    'quantity': 'sum'
}).round(2)
product_stats.columns = ['总收入', '平均收入', '订单数', '总数量']
print(product_stats.sort_values('总收入', ascending=False))

# 4.3 按地区分析
print("\n=== 按地区分析 ===")
region_stats = df_clean.groupby('region').agg({
    'revenue': ['sum', 'mean'],
    'discount': 'mean'
}).round(2)
region_stats.columns = ['总收入', '平均收入', '平均折扣']
print(region_stats.sort_values('总收入', ascending=False))

# 4.4 按月份分析
print("\n=== 按月份分析 ===")
monthly_revenue = df_clean.groupby('month')['revenue'].sum()
print(monthly_revenue)

print("\n" + "=" * 60)
print("5. 数据可视化")
print("=" * 60)

# 创建图表
fig, axes = plt.subplots(2, 3, figsize=(18, 12))

# 5.1 按产品的销售额
product_revenue = df_clean.groupby('product')['revenue'].sum().sort_values(ascending=True)
axes[0, 0].barh(product_revenue.index, product_revenue.values, color='steelblue')
axes[0, 0].set_title('按产品销售额')
axes[0, 0].set_xlabel('销售额 (¥)')

# 5.2 按地区的销售额饼图
region_revenue = df_clean.groupby('region')['revenue'].sum()
axes[0, 1].pie(region_revenue, labels=region_revenue.index, autopct='%1.1f%%',
               colors=['#ff9999', '#66b3ff', '#99ff99', '#ffcc99'])
axes[0, 1].set_title('按地区销售额占比')

# 5.3 月度销售趋势
monthly_data = df_clean.groupby('month')['revenue'].sum()
axes[0, 2].plot(monthly_data.index, monthly_data.values, 'o-', linewidth=2, markersize=8)
axes[0, 2].set_title('月度销售趋势')
axes[0, 2].set_xlabel('月份')
axes[0, 2].set_ylabel('销售额 (¥)')
axes[0, 2].set_xticks(range(1, 13))
axes[0, 2].grid(True, alpha=0.3)

# 5.4 销售额分布
axes[1, 0].hist(df_clean['revenue'], bins=30, edgecolor='black', alpha=0.7)
axes[1, 0].axvline(df_clean['revenue'].mean(), color='red', linestyle='--', label='均值')
axes[1, 0].axvline(df_clean['revenue'].median(), color='green', linestyle='--', label='中位数')
axes[1, 0].set_title('销售额分布')
axes[1, 0].set_xlabel('销售额 (¥)')
axes[1, 0].set_ylabel('频次')
axes[1, 0].legend()

# 5.5 按类别和地区的热力图
pivot = df_clean.pivot_table(values='revenue', index='category', columns='region', aggfunc='sum')
sns.heatmap(pivot, annot=True, fmt=',.0f', cmap='YlOrRd', ax=axes[1, 1])
axes[1, 1].set_title('类别-地区销售热力图')

# 5.6 折扣与销售额的关系
axes[1, 2].scatter(df_clean['discount'], df_clean['revenue'], alpha=0.5)
axes[1, 2].set_title('折扣与销售额关系')
axes[1, 2].set_xlabel('折扣率')
axes[1, 2].set_ylabel('销售额 (¥)')

plt.tight_layout()
plt.savefig('sales_analysis.png', dpi=150, bbox_inches='tight')
plt.show()

print("\n" + "=" * 60)
print("6. 业务洞察")
print("=" * 60)

# 找出表现最好的产品
best_product = df_clean.groupby('product')['revenue'].sum().idxmax()
print(f"\n最畅销产品: {best_product}")

# 找出表现最好的地区
best_region = df_clean.groupby('region')['revenue'].sum().idxmax()
print(f"最佳销售地区: {best_region}")

# 找出最佳销售月份
best_month = df_clean.groupby('month')['revenue'].sum().idxmax()
print(f"最佳销售月份: {best_month}月")

# 折扣效果分析
high_discount = df_clean[df_clean['discount'] > 0.2]['revenue'].mean()
low_discount = df_clean[df_clean['discount'] <= 0.2]['revenue'].mean()
print(f"\n高折扣(>20%)平均销售额: ¥{high_discount:,.0f}")
print(f"低折扣(≤20%)平均销售额: ¥{low_discount:,.0f}")

print("\n项目 A 完成！")
```

---

## 项目 B：向量相似度搜索

### 项目目标

用 NumPy 实现向量相似度计算，模拟简单的语义搜索。

### 完整代码

```python
"""
项目 B：向量相似度搜索
目标：实现简单的语义搜索系统
"""

import numpy as np
from typing import List, Tuple

print("=" * 60)
print("向量相似度搜索")
print("=" * 60)

# ========== 1. 定义相似度函数 ==========
print("\n1. 实现相似度计算函数")
print("-" * 40)

def cosine_similarity(v1: np.ndarray, v2: np.ndarray) -> float:
    """计算两个向量的余弦相似度"""
    dot_product = np.dot(v1, v2)
    norm1 = np.linalg.norm(v1)
    norm2 = np.linalg.norm(v2)
    return dot_product / (norm1 * norm2)

def euclidean_distance(v1: np.ndarray, v2: np.ndarray) -> float:
    """计算两个向量的欧几里得距离"""
    return np.linalg.norm(v1 - v2)

# 测试
v1 = np.array([1, 0, 0])
v2 = np.array([0, 1, 0])
v3 = np.array([1, 1, 0])

print(f"v1 = {v1}")
print(f"v2 = {v2}")
print(f"v3 = {v3}")
print(f"\n余弦相似度:")
print(f"  v1 和 v2: {cosine_similarity(v1, v2):.4f} (垂直)")
print(f"  v1 和 v3: {cosine_similarity(v1, v3):.4f} (45度)")
print(f"  v1 和 v1: {cosine_similarity(v1, v1):.4f} (相同)")

# ========== 2. 模拟文档向量 ==========
print("\n2. 创建文档向量数据库")
print("-" * 40)

# 模拟一个简单的词嵌入
# 实际中会使用 Word2Vec, GloVe 或 BERT 等模型
documents = [
    "Python 是一种流行的编程语言",
    "机器学习需要大量数据",
    "深度学习是人工智能的重要分支",
    "Python 广泛用于数据科学",
    "神经网络可以学习复杂模式",
    "数据分析需要 Python 和 pandas",
    "自然语言处理是 AI 的应用领域",
    "TensorFlow 和 PyTorch 是深度学习框架",
]

# 模拟文档向量（实际中应该用真实的 embedding 模型）
np.random.seed(42)
embedding_dim = 128
document_vectors = np.random.randn(len(documents), embedding_dim)

# 让相关文档的向量更接近（模拟）
# Python 相关：0, 3, 5
# 深度学习相关：2, 4, 7
# AI 相关：2, 6
document_vectors[3] = document_vectors[0] + np.random.randn(embedding_dim) * 0.3
document_vectors[5] = document_vectors[0] + np.random.randn(embedding_dim) * 0.4
document_vectors[4] = document_vectors[2] + np.random.randn(embedding_dim) * 0.3
document_vectors[7] = document_vectors[2] + np.random.randn(embedding_dim) * 0.4

print(f"文档数量: {len(documents)}")
print(f"向量维度: {embedding_dim}")

# ========== 3. 实现搜索函数 ==========
print("\n3. 实现搜索功能")
print("-" * 40)

def search(query_vector: np.ndarray,
           document_vectors: np.ndarray,
           documents: List[str],
           top_k: int = 3) -> List[Tuple[int, str, float]]:
    """
    搜索最相似的文档

    Args:
        query_vector: 查询向量
        document_vectors: 文档向量矩阵
        documents: 文档列表
        top_k: 返回前 k 个结果

    Returns:
        [(索引, 文档, 相似度), ...]
    """
    similarities = []
    for i, doc_vec in enumerate(document_vectors):
        sim = cosine_similarity(query_vector, doc_vec)
        similarities.append((i, documents[i], sim))

    # 按相似度降序排序
    similarities.sort(key=lambda x: x[2], reverse=True)

    return similarities[:top_k]

# ========== 4. 测试搜索 ==========
print("\n4. 测试搜索")
print("-" * 40)

# 模拟查询（使用第一个文档的向量作为查询）
query_idx = 0
query_vector = document_vectors[query_idx]

print(f"查询: '{documents[query_idx]}'")
print(f"\n搜索结果:")

results = search(query_vector, document_vectors, documents, top_k=5)
for i, (idx, doc, sim) in enumerate(results, 1):
    print(f"  {i}. [{sim:.4f}] {doc}")

# ========== 5. 批量相似度计算 ==========
print("\n5. 文档相似度矩阵")
print("-" * 40)

# 计算所有文档之间的相似度
def compute_similarity_matrix(vectors: np.ndarray) -> np.ndarray:
    """计算相似度矩阵"""
    n = len(vectors)
    sim_matrix = np.zeros((n, n))
    for i in range(n):
        for j in range(n):
            sim_matrix[i, j] = cosine_similarity(vectors[i], vectors[j])
    return sim_matrix

# 更高效的实现（使用矩阵运算）
def compute_similarity_matrix_fast(vectors: np.ndarray) -> np.ndarray:
    """使用矩阵运算计算相似度矩阵（更高效）"""
    # 归一化
    norms = np.linalg.norm(vectors, axis=1, keepdims=True)
    normalized = vectors / norms
    # 矩阵乘法得到相似度
    return normalized @ normalized.T

sim_matrix = compute_similarity_matrix_fast(document_vectors)

# 可视化
import matplotlib.pyplot as plt

plt.figure(figsize=(10, 8))
plt.imshow(sim_matrix, cmap='YlOrRd')
plt.colorbar(label='Cosine Similarity')
plt.xticks(range(len(documents)), [f'Doc {i}' for i in range(len(documents))], rotation=45)
plt.yticks(range(len(documents)), [f'Doc {i}' for i in range(len(documents))])
plt.title('Document Similarity Matrix')
plt.tight_layout()
plt.savefig('similarity_matrix.png', dpi=150)
plt.show()

# 找出最相似的文档对
print("\n最相似的文档对:")
n = len(documents)
pairs = []
for i in range(n):
    for j in range(i + 1, n):
        pairs.append((i, j, sim_matrix[i, j]))

pairs.sort(key=lambda x: x[2], reverse=True)
for i, j, sim in pairs[:5]:
    print(f"  [{sim:.4f}] Doc {i} <-> Doc {j}")
    print(f"    - {documents[i]}")
    print(f"    - {documents[j]}")

# ========== 6. 扩展：简单的推荐系统 ==========
print("\n6. 基于相似度的推荐")
print("-" * 40)

def recommend(doc_idx: int,
              sim_matrix: np.ndarray,
              documents: List[str],
              top_k: int = 3) -> List[Tuple[int, str, float]]:
    """基于文档相似度推荐相关文档"""
    similarities = sim_matrix[doc_idx]
    indices = np.argsort(similarities)[::-1]  # 降序

    results = []
    for idx in indices:
        if idx != doc_idx:  # 排除自己
            results.append((idx, documents[idx], similarities[idx]))
            if len(results) >= top_k:
                break

    return results

# 测试推荐
print(f"如果你阅读了: '{documents[0]}'")
print(f"你可能也会喜欢:")
recommendations = recommend(0, sim_matrix, documents, top_k=3)
for idx, doc, sim in recommendations:
    print(f"  - [{sim:.4f}] {doc}")

print("\n项目 B 完成！")
```

---

## 项目总结

### 项目 A 学到的技能

- Pandas 数据加载和清洗
- 数据聚合和统计分析
- Matplotlib 数据可视化
- 业务洞察提取

### 项目 B 学到的技能

- NumPy 向量运算
- 余弦相似度计算
- 简单的搜索算法
- 相似度矩阵计算

### 下一步建议

1. **数据分析**：找一个真实的数据集（Kaggle 上有很多），进行完整的 EDA
2. **向量搜索**：使用真实的词嵌入模型（如 Sentence-BERT）
3. **尝试更多**：结合两个项目，比如分析向量搜索的效果

---

## ➡️ 下一步

完成项目后，进入 [10-自测清单.md](./10-自测清单.md) 检验学习成果！

