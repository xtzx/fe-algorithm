# 🖼️ 09 - 多模态应用

> 图像理解、语音交互、多模态 RAG

---

## 图像理解

### GPT-4V / GPT-4o

```python
from openai import OpenAI
import base64

client = OpenAI()

def encode_image(image_path: str) -> str:
    """将图片编码为 base64"""
    with open(image_path, "rb") as f:
        return base64.b64encode(f.read()).decode("utf-8")

def analyze_image(image_path: str, question: str = "这张图片里有什么？"):
    """使用 GPT-4o 分析图片"""
    base64_image = encode_image(image_path)

    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": question},
                    {
                        "type": "image_url",
                        "image_url": {
                            "url": f"data:image/jpeg;base64,{base64_image}"
                        }
                    }
                ]
            }
        ],
        max_tokens=1000
    )

    return response.choices[0].message.content

# 使用 URL
def analyze_image_url(image_url: str, question: str):
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": question},
                    {"type": "image_url", "image_url": {"url": image_url}}
                ]
            }
        ]
    )
    return response.choices[0].message.content

# 测试
result = analyze_image("photo.jpg", "请描述这张图片的内容，并识别其中的文字")
print(result)
```

### Claude 3 Vision

```python
import anthropic
import base64

client = anthropic.Anthropic()

def analyze_with_claude(image_path: str, question: str):
    """使用 Claude 3 分析图片"""
    with open(image_path, "rb") as f:
        image_data = base64.standard_b64encode(f.read()).decode("utf-8")

    # 推断媒体类型
    if image_path.endswith(".png"):
        media_type = "image/png"
    elif image_path.endswith(".gif"):
        media_type = "image/gif"
    elif image_path.endswith(".webp"):
        media_type = "image/webp"
    else:
        media_type = "image/jpeg"

    response = client.messages.create(
        model="claude-3-5-sonnet-20241022",
        max_tokens=1024,
        messages=[
            {
                "role": "user",
                "content": [
                    {
                        "type": "image",
                        "source": {
                            "type": "base64",
                            "media_type": media_type,
                            "data": image_data
                        }
                    },
                    {"type": "text", "text": question}
                ]
            }
        ]
    )

    return response.content[0].text

result = analyze_with_claude("chart.png", "请分析这个图表的趋势")
print(result)
```

### 本地多模态模型（Qwen-VL）

```python
from transformers import Qwen2VLForConditionalGeneration, AutoProcessor
from qwen_vl_utils import process_vision_info
import torch

# 加载模型
model = Qwen2VLForConditionalGeneration.from_pretrained(
    "Qwen/Qwen2-VL-2B-Instruct",
    torch_dtype=torch.float16,
    device_map="auto"
)
processor = AutoProcessor.from_pretrained("Qwen/Qwen2-VL-2B-Instruct")

def analyze_local(image_path: str, question: str):
    """使用本地 Qwen-VL 分析图片"""
    messages = [
        {
            "role": "user",
            "content": [
                {"type": "image", "image": image_path},
                {"type": "text", "text": question}
            ]
        }
    ]

    text = processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
    image_inputs, video_inputs = process_vision_info(messages)

    inputs = processor(
        text=[text],
        images=image_inputs,
        videos=video_inputs,
        padding=True,
        return_tensors="pt"
    ).to("cuda")

    outputs = model.generate(**inputs, max_new_tokens=256)
    response = processor.batch_decode(outputs, skip_special_tokens=True)[0]

    return response

result = analyze_local("document.png", "请提取这个文档中的关键信息")
print(result)
```

---

## 语音交互

### Whisper（语音转文字）

```python
from openai import OpenAI

client = OpenAI()

def transcribe_audio(audio_path: str, language: str = "zh") -> str:
    """语音转文字"""
    with open(audio_path, "rb") as audio_file:
        transcript = client.audio.transcriptions.create(
            model="whisper-1",
            file=audio_file,
            language=language,
            response_format="text"
        )
    return transcript

# 带时间戳
def transcribe_with_timestamps(audio_path: str) -> dict:
    """语音转文字（带时间戳）"""
    with open(audio_path, "rb") as audio_file:
        transcript = client.audio.transcriptions.create(
            model="whisper-1",
            file=audio_file,
            response_format="verbose_json",
            timestamp_granularities=["segment"]
        )
    return transcript

# 本地 Whisper
import whisper

def local_transcribe(audio_path: str):
    """使用本地 Whisper 模型"""
    model = whisper.load_model("base")  # tiny, base, small, medium, large
    result = model.transcribe(audio_path, language="zh")
    return result["text"]

# 测试
text = transcribe_audio("recording.mp3")
print(f"转录结果: {text}")
```

### TTS（文字转语音）

```python
from openai import OpenAI
from pathlib import Path

client = OpenAI()

def text_to_speech(text: str, output_path: str, voice: str = "alloy"):
    """文字转语音

    voice 可选: alloy, echo, fable, onyx, nova, shimmer
    """
    response = client.audio.speech.create(
        model="tts-1",  # 或 tts-1-hd
        voice=voice,
        input=text
    )

    response.stream_to_file(output_path)
    print(f"音频已保存到 {output_path}")

# 使用
text_to_speech("你好，欢迎使用语音助手！", "output.mp3", voice="nova")

# 本地 TTS（使用 edge-tts）
import edge_tts
import asyncio

async def local_tts(text: str, output_path: str, voice: str = "zh-CN-XiaoxiaoNeural"):
    """使用 Edge TTS（免费）"""
    communicate = edge_tts.Communicate(text, voice)
    await communicate.save(output_path)

# 运行
asyncio.run(local_tts("你好世界", "output.mp3"))
```

### 语音对话系统

```python
import sounddevice as sd
import numpy as np
from scipy.io import wavfile
import tempfile

class VoiceAssistant:
    """简单的语音助手"""

    def __init__(self):
        self.client = OpenAI()
        self.sample_rate = 16000

    def record_audio(self, duration: int = 5) -> str:
        """录制音频"""
        print(f"录音中... ({duration}秒)")
        audio = sd.rec(
            int(duration * self.sample_rate),
            samplerate=self.sample_rate,
            channels=1,
            dtype=np.int16
        )
        sd.wait()

        # 保存临时文件
        temp_file = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
        wavfile.write(temp_file.name, self.sample_rate, audio)
        return temp_file.name

    def transcribe(self, audio_path: str) -> str:
        """语音转文字"""
        with open(audio_path, "rb") as f:
            result = self.client.audio.transcriptions.create(
                model="whisper-1",
                file=f,
                language="zh"
            )
        return result.text

    def chat(self, user_message: str) -> str:
        """与 LLM 对话"""
        response = self.client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "你是一个友好的语音助手，回答要简洁。"},
                {"role": "user", "content": user_message}
            ]
        )
        return response.choices[0].message.content

    def speak(self, text: str):
        """文字转语音并播放"""
        response = self.client.audio.speech.create(
            model="tts-1",
            voice="nova",
            input=text
        )

        # 保存并播放
        temp_file = tempfile.NamedTemporaryFile(suffix=".mp3", delete=False)
        response.stream_to_file(temp_file.name)

        # 使用 playsound 播放（需要 pip install playsound）
        from playsound import playsound
        playsound(temp_file.name)

    def run(self):
        """运行语音助手"""
        print("语音助手已启动！说 '退出' 结束对话。")

        while True:
            # 录音
            audio_path = self.record_audio(5)

            # 转文字
            user_text = self.transcribe(audio_path)
            print(f"你: {user_text}")

            if "退出" in user_text:
                print("再见！")
                break

            # 获取回复
            response = self.chat(user_text)
            print(f"助手: {response}")

            # 语音回复
            self.speak(response)

# 运行
assistant = VoiceAssistant()
assistant.run()
```

---

## 多模态 RAG

### 图片 + 文本混合检索

```python
from sentence_transformers import SentenceTransformer
import torch
from PIL import Image
from transformers import CLIPProcessor, CLIPModel

class MultimodalRAG:
    """多模态 RAG：支持图片和文本"""

    def __init__(self):
        # 文本 embedding
        self.text_model = SentenceTransformer('all-MiniLM-L6-v2')

        # CLIP 模型（图文对齐）
        self.clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
        self.clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")

        self.documents = []  # 存储文档
        self.images = []     # 存储图片

    def add_text(self, text: str, metadata: dict = None):
        """添加文本文档"""
        embedding = self.text_model.encode(text)
        self.documents.append({
            "type": "text",
            "content": text,
            "embedding": embedding,
            "metadata": metadata or {}
        })

    def add_image(self, image_path: str, caption: str = "", metadata: dict = None):
        """添加图片"""
        image = Image.open(image_path)

        # 使用 CLIP 获取图片 embedding
        inputs = self.clip_processor(images=image, return_tensors="pt")
        with torch.no_grad():
            image_features = self.clip_model.get_image_features(**inputs)

        embedding = image_features.squeeze().numpy()

        self.images.append({
            "type": "image",
            "path": image_path,
            "caption": caption,
            "embedding": embedding,
            "metadata": metadata or {}
        })

    def search(self, query: str, top_k: int = 5, search_type: str = "all"):
        """搜索

        search_type: "text", "image", "all"
        """
        results = []

        if search_type in ["text", "all"]:
            # 文本搜索
            query_emb = self.text_model.encode(query)
            for doc in self.documents:
                score = np.dot(query_emb, doc["embedding"]) / (
                    np.linalg.norm(query_emb) * np.linalg.norm(doc["embedding"])
                )
                results.append({**doc, "score": float(score)})

        if search_type in ["image", "all"]:
            # 图片搜索（使用 CLIP 文本编码）
            inputs = self.clip_processor(text=[query], return_tensors="pt", padding=True)
            with torch.no_grad():
                text_features = self.clip_model.get_text_features(**inputs)
            query_emb = text_features.squeeze().numpy()

            for img in self.images:
                score = np.dot(query_emb, img["embedding"]) / (
                    np.linalg.norm(query_emb) * np.linalg.norm(img["embedding"])
                )
                results.append({**img, "score": float(score)})

        # 排序
        results.sort(key=lambda x: x["score"], reverse=True)
        return results[:top_k]

    def query(self, question: str, top_k: int = 5):
        """RAG 查询"""
        # 检索
        results = self.search(question, top_k)

        # 构造上下文
        context_parts = []
        for r in results:
            if r["type"] == "text":
                context_parts.append(f"[文档] {r['content']}")
            else:
                context_parts.append(f"[图片] {r['caption']} (路径: {r['path']})")

        context = "\n\n".join(context_parts)

        # 如果有图片，使用多模态模型
        image_paths = [r["path"] for r in results if r["type"] == "image"]

        if image_paths:
            # 使用 GPT-4o 同时理解文本和图片
            return self._multimodal_generate(question, context, image_paths)
        else:
            return self._text_generate(question, context)

    def _text_generate(self, question: str, context: str) -> str:
        """纯文本生成"""
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "基于提供的上下文回答问题。"},
                {"role": "user", "content": f"上下文:\n{context}\n\n问题: {question}"}
            ]
        )
        return response.choices[0].message.content

    def _multimodal_generate(self, question: str, context: str, image_paths: list) -> str:
        """多模态生成"""
        content = [{"type": "text", "text": f"上下文:\n{context}\n\n问题: {question}"}]

        for path in image_paths[:4]:  # 最多 4 张图
            base64_image = encode_image(path)
            content.append({
                "type": "image_url",
                "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}
            })

        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "基于提供的文本和图片回答问题。"},
                {"role": "user", "content": content}
            ]
        )
        return response.choices[0].message.content


# 使用示例
rag = MultimodalRAG()

# 添加文档
rag.add_text("Python 是一种高级编程语言...")
rag.add_text("机器学习是 AI 的一个分支...")

# 添加图片
rag.add_image("architecture.png", caption="系统架构图")
rag.add_image("chart.png", caption="性能对比图表")

# 查询
result = rag.query("请解释系统架构")
print(result)
```

---

## 练习题

### 练习 1：图片问答机器人

```python
# 任务：创建一个能够：
# 1. 接收用户上传的图片
# 2. 回答关于图片的问题
# 3. 支持多轮对话
```

### 练习 2：语音转文字应用

```python
# 任务：创建一个会议记录工具：
# 1. 录制会议音频
# 2. 转录为文字
# 3. 自动生成会议摘要
```

### 练习 3：多模态知识库

```python
# 任务：构建一个产品说明书知识库：
# 1. 支持 PDF（包含图片）
# 2. 支持独立图片
# 3. 能够回答关于产品的问题
```

---

## 小结

```
本节要点：
1. 图像理解：GPT-4o / Claude 3 / Qwen-VL
2. 语音交互：Whisper（ASR）+ TTS
3. 多模态 RAG：CLIP + 混合检索
4. 应用场景：问答、会议记录、知识库
```

---

## ➡️ 下一步

继续 [10-LLM应用安全.md](./10-LLM应用安全.md)

