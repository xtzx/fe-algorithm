...
> 使用 LLaMA-Factory 和 Unsloth 完成微调
...
```
LLaMA-Factory 是一个一站式大模型微调平台：
- 支持 100+ 模型
- 多种微调方法（LoRA、QLoRA、全量）
- Web UI 和命令行
- 丰富的数据集支持
- 完整的训练监控
...
model = FastLanguageModel.get_peft_model(
    ...
    lora_dropout=0,
    bias="none",
    use_gradient_checkpointing="unsloth",  # 优化显存
    random_state=3407,
...
```
本节要点：
1. LLaMA-Factory：一站式微调平台，Web UI 友好
2. Unsloth：高性能微调库，速度快、显存省
3. 完整流程：数据准备 → 配置 → 训练 → 评估 → 部署
4. 常见问题：显存、稳定性、效果优化
```
