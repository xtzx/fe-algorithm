...
import { sort, sortInPlace, sortBinary, insertSorted, sortWithStats, meta } from '../src/index';
...
assert(verifySorted(sortedRandom, numberAsc).passed, '随机数据排序 - 有序性');
assert(verifyPermutation(randomData, sortedRandom).passed, '随机数据排序 - 置换性');
...
const { comparisons: sortedComparisons } = sortWithStats(sortedData, numberAsc);
assertEqual(sortedComparisons, 9, '已排序数组比较次数 = n-1');

const nearlySorted = [1, 2, 4, 3, 5, 6, 8, 7, 9, 10];
const { comparisons: nearlyComparisons, moves: nearlyMoves } = sortWithStats(nearlySorted, numberAsc);
assert(nearlyComparisons < 20, `近乎有序数据比较次数少 (${nearlyComparisons})`);
console.log(`  近乎有序数据: 比较=${nearlyComparisons}, 移动=${nearlyMoves}`);
...
const binaryData = generateNumbers(50, 'random');
const binarySorted = sortBinary(binaryData, numberAsc);

assert(verifySorted(binarySorted, numberAsc).passed, '二分插入排序 - 有序性');
assert(verifyPermutation(binaryData, binarySorted).passed, '二分插入排序 - 置换性');
...
// 插入多个元素
insertSorted(list, 5, numberAsc);
assertEqual(list, [5], '插入第一个元素');

insertSorted(list, 3, numberAsc);
assertEqual(list, [3, 5], '插入到开头');

insertSorted(list, 8, numberAsc);
assertEqual(list, [3, 5, 8], '插入到末尾');

insertSorted(list, 4, numberAsc);
assertEqual(list, [3, 4, 5, 8], '插入到中间');

insertSorted(list, 4, numberAsc);
assertEqual(list, [3, 4, 4, 5, 8], '插入重复元素');
...
assert(meta.stable === true, '元信息 - 稳定');
assert(meta.inPlace === true, '元信息 - 原地排序');
assert(meta.timeComplexity.best === 'O(n)', '元信息 - 最好时间复杂度 O(n)');
