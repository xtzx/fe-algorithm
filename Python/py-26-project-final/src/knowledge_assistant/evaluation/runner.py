...
from knowledge_assistant.evaluation.metrics import Metrics, MetricResult
...
class EvaluationResults:
    ...
    def summary(self) -> str:
        ...
        lines = [
            ...
            f"通过: {self.passed_cases} ({pass_rate:.1f}%)",
...
class EvaluationRunner:
    ...
    Usage:
        ...
        def model_fn(input_text):
            return rag_generator.generate(input_text).answer
    ...
    def run(
        ...
        logger.info("evaluation_started", dataset=dataset.name, cases=len(dataset))
        ...
        for case in dataset:
            try:
                ...
                # 计算指标
                case_metrics: Dict[str, MetricResult] = {}
    ...
    def run_single(
        ...
        """运行单个测试用例"""
        temp_dataset = EvaluationDataset("single", "")
        temp_dataset.add(case)
        
        results = self.run(model_fn, temp_dataset)
...
class RAGEvaluationRunner(EvaluationRunner):
    ...
    def run_rag(
        ...
        def context_fn(case: TestCase) -> str:
