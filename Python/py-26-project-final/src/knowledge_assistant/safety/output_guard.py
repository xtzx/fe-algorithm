...
class OutputGuard:
    ...
    Usage:
        ...
        # PII 过滤
        safe_text = guard.remove_pii("Contact john@example.com")
        # "Contact [EMAIL]"
    ...
    PII_PATTERNS = {
        ...
        PIIType.PHONE: r'\b(?:\+?86[-\s]?)?1[3-9]\d{9}\b|\b(?:\+?1[-.\s]?)?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}\b',
    ...
    HARMFUL_PATTERNS = {
        "violence": [
            r'\b(kill|murder|harm|attack|violent|暴力|杀|打死)\w*\b',
        ],
        "hate": [
            r'\b(hate|racist|sexist|discriminat|歧视)\w*\b',
        ],
        "self_harm": [
            r'\b(suicide|self[-\s]?harm|自杀|自残)\b',
        ],
        "illegal": [
            r'\b(drug|cocaine|heroin|毒品)\w*\b',
        ],
    }
    ...
    def moderate(self, text: str) -> ModerationResult:
        ...
        for category, patterns in self._harmful_patterns.items():
            ...
            # 简单评分：匹配数/总模式数
            score = min(1.0, match_count / max(len(patterns), 1))
        ...
        reason = f"标记的类别: {', '.join(reasons)}" if reasons else None
    ...
    def process_output(
        ...
        if moderate:
            moderation_result = self.moderate(processed)
        
        return processed, moderation_result
...
class HallucinationDetector:
    ...
    def __init__(self):
        ...
        self._overconfidence_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.OVERCONFIDENCE_INDICATORS
        ]
    ...
    def analyze(self, text: str, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        overconfidence_count = sum(
            len(p.findall(text)) for p in self._overconfidence_patterns
        )
