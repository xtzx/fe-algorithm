...
from knowledge_assistant.rag.index import IndexEntry, VectorIndex
...
class Retriever:
    ...
    def search_with_citations(
        ...
        for i, result in enumerate(results):
            citations.append(Citation(
                ...
                chunk_id=result.chunk_id or f"chunk_{i}",
    ...
    def format_citations(self, citations: List[Citation]) -> str:
        ...
        for i, cite in enumerate(citations, 1):
            line = f"[{i}] {cite.source}"
            if cite.page:
                line += f", page {cite.page}"
            if cite.section:
                line += f", section: {cite.section}"
            lines.append(line)
        return "\n".join(lines)
